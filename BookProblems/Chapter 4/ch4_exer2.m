...
for k=1:N_bus
    for l=1:N_bus
        if k==l
            for m=1:N_bus
                Y_ma(k,k)=Y_ma(k,k)+A(k,m)^2*y_ma(k,m)+1i*b_sh_ma(k,m);
            end
            Y_ma(k,k)=Y_ma(k,k)+1i*b_sh_bus(k); % there is no bus shunt so this is just symbolic here
        else
            Y_ma(k,l)=-A(k,l)*exp(-1i*phi(k,l))*y_ma(k,l);
        end
    end
end
          
Y=zeros(N_bus,N_bus);
%Y, off-diagonal elements
for k=1:N_bra
    Y(from(k),to(k))= -A(from(k),to(k))*y(k); %does not includ transformer pahse shift because it does not exist in this problem
    Y(to(k),from(k))=-A(to(k),from(k))*y(k);
end
%Y, diagonal element
for m=1 : N_bus
    for n = 1:N_bra
        if from(n)==m % for the branches that leave from this bus
            Y(m,m)= Y(m,m)+(a(n)^2)*y(n)+1i*b_sh(n); %does not includ transformer pahse shift because it does not exist in this problem
        elseif to(n)==m % for the branches that reach this bus 
            Y(m,m)= Y(m,m)+y(n)+1i*b_sh(n);
        end
    end 
end

fprintf("we test the difference now\n")
Y=Y_ma;
...
for it=1:15
    % delta z:
    %fill the power formula vector h()
        %declaring and clean the function vector h(x)
        h=zeros(N_mes,1);
